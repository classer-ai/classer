...
|-------|---------------------|----------------------|----------|
| **DeepSeek-V4-Flash** | $0.14 | $0.28 | Azure AI Foundry |

Token counts come straight from the upstream `usage` chunk on the streaming
response, so the bill always matches what the model actually generated. See
[Generative LLM](/api-reference/generate) for endpoint details.

<Note>
DeepSeek-V4-Flash is **streaming-only** through Classer — set `stream: true`
on every request. The proxy auto-injects `stream_options.include_usage=true`
so the final SSE chunk carries the token count we bill from.
</Note>
