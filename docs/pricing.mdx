...
Fast and Standard are set via the `speed` parameter on `/v1/classify` and `/v1/tag`. Fast Batch pricing applies automatically when using the [async batch endpoints](/api-reference/batch).
