...
description: "Speed tiers, pricing, free tier, and rate limits"
---
...
Fast and Standard are set via the `speed` parameter on `/v1/classify` and `/v1/tag`. Async Batch pricing applies automatically when using the [async batch endpoints](/api-reference/batch).

## Free tier

Every account gets **10M free tokens/month** — no credit card required. Free tokens work on all speed tiers.
