---
title: Classify
api: POST /v1/classify
description: "Classify text into exactly one label"
---

## Request

<ParamField body="text" type="string">
  The text to classify (max 1M tokens). Required unless an image is provided.
</ParamField>

<ParamField body="image_url" type="string or string[]">
  Image URL(s) to classify. Images above 4MP (2048×2048) are resized. Max 5 images.
</ParamField>

<ParamField body="image_base64" type="string or string[]">
  Base64-encoded image(s) to classify. Images above 4MP (2048×2048) are resized. Max 5 images, ~20MB each.
</ParamField>

<ParamField body="labels" type="string[]">
  List of possible labels (1–200, max 100 characters each). Required unless `classifier` is provided.
</ParamField>

<ParamField body="classifier" type="string">
  Saved classifier name (e.g. `"support-tickets"`) or a pinned version (`"support-tickets@v2"`). Alternative to providing `labels` inline.
</ParamField>

<ParamField body="descriptions" type="object">
  Optional descriptions for each label to improve accuracy. Keys are label names, values are description strings.
</ParamField>

<ParamField body="examples" type="object">
  Optional per-label example texts to improve accuracy. Keys are label names, values are arrays of example strings.
</ParamField>

<ParamField body="text_type" type="string">
  Optional context for prompt tuning (e.g. `"app review"`, `"support ticket"`)
</ParamField>

<ParamField body="speed" type="string" default="standard">
  Speed tier: `standard` (&lt;1s, $0.20/1M tokens) or `fast` (&lt;200ms, $0.60/1M tokens)
</ParamField>

<ParamField body="cache" type="boolean" default="true">
  Set to `false` to bypass cache and force a fresh classification
</ParamField>

## Response

<ResponseField name="label" type="string">
  The predicted label
</ResponseField>

<ResponseField name="confidence" type="number">
  Confidence score between 0 and 1 (rounded to 4 decimal places)
</ResponseField>

<ResponseField name="tokens" type="number">
  Total tokens used
</ResponseField>

<ResponseField name="visual_tokens" type="number">
  Number of visual (image) tokens used. Only present when an image is provided.
</ResponseField>

<ResponseField name="latency_ms" type="number">
  Processing time in milliseconds
</ResponseField>

<ResponseField name="cached" type="boolean">
  Whether the response was served from cache
</ResponseField>

## Examples

### Basic classification

<CodeGroup>

```python Python
import classer

result = classer.classify(
    text="I can't log in to my account",
    labels=["billing", "technical", "sales"]
)

print(result.label)      # "technical"
print(result.confidence) # 0.94
```

```typescript TypeScript
import { classify } from "classer-ai";

const result = await classify({
  text: "I can't log in to my account",
  labels: ["billing", "technical", "sales"]
});

console.log(result.label);      // "technical"
console.log(result.confidence); // 0.94
```

```bash cURL
curl -X POST https://api.classer.ai/v1/classify \
  -H "Authorization: Bearer $CLASSER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "I cannot log in to my account",
    "labels": ["billing", "technical", "sales"]
  }'
```

</CodeGroup>

### With descriptions

Adding descriptions improves accuracy for ambiguous labels:

<CodeGroup>

```python Python
result = classer.classify(
    text="We need pricing for 500 users",
    labels=["hot", "warm", "cold"],
    descriptions={
        "hot": "Ready to buy, asking for pricing or demo",
        "warm": "Interested but still exploring",
        "cold": "Just browsing, no clear intent"
    }
)

print(result.label)  # "hot"
```

```typescript TypeScript
const result = await classify({
  text: "We need pricing for 500 users",
  labels: ["hot", "warm", "cold"],
  descriptions: {
    hot: "Ready to buy, asking for pricing or demo",
    warm: "Interested but still exploring",
    cold: "Just browsing, no clear intent"
  }
});

console.log(result.label);  // "hot"
```

```bash cURL
curl -X POST https://api.classer.ai/v1/classify \
  -H "Authorization: Bearer $CLASSER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "We need pricing for 500 users",
    "labels": ["hot", "warm", "cold"],
    "descriptions": {
      "hot": "Ready to buy, asking for pricing or demo",
      "warm": "Interested but still exploring",
      "cold": "Just browsing, no clear intent"
    }
  }'
```

</CodeGroup>

<ResponseExample>
```json
{
  "label": "technical",
  "confidence": 0.9423,
  "tokens": 142,
  "latency_ms": 87,
  "cached": false
}
```
</ResponseExample>
